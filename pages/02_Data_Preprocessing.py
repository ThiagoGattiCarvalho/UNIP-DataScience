...
from sklearn.preprocessing import StandardScaler, MinMaxScaler, QuantileTransformer, Normalizer # Idem para os scalers.
...
preprocessing1, preprocessing2, preprocessing3, preprocessing4, preprocessing5, preprocessing6, preprocessing7, preprocessing8, preprocessing9 = st.tabs(['Preenchimento de Dados', 'Codificação de Dados', 'Escalonamento de Dados', 'Aumento de Dados', 'Redução de Dados', 'Importância de Dados', 'Correlações', 'Análise Bivariada', 'EDA - Análise Exploratória de Dados - Depois'])
...
with preprocessing3:                                                                # Exatamente a mesma estrutura dos cases anteriores
    ...
    with preprocessing3_1:
        lista_scalers = ['Nenhum', StandardScaler(), MinMaxScaler(), QuantileTransformer(), Normalizer()]
...
with preprocessing5:                                                                # Agora a estrutura é apenas parecida às anteriores.
    ...
    with preprocessing5_1:                                                          # Novo aqui.
        coluna_y = [st.selectbox('Selecione o alvo:', colunas_numericas)]
        y = df[coluna_y]
...
with preprocessing6:                                                                # Mesma coisa ... de novo.
    ...
    plt.title('Importância Média dos Features')
    plt.xlabel('Importância Média')
    plt.ylabel('Feature')
    # plt.tight_layout()
    st.pyplot(plt.gcf(), use_container_width=False, clear_figure=False)             # clear_figure=False não deixa o Streamlit apagar a figura.
...
with preprocessing8:
    ...
    if grid:
        ...
        st.pyplot(plt.gcf(),                                                # Streamlit, mostre a figura.
                  use_container_width=False,                                # Não manipule o tamanho da figura para ocupar a largura da tela.
